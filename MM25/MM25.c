...
double globCommTime;
double dist_time,rd_time,bcast_time,cmp_time;
...
void mypdgemm_summa(int n,double* A,double* B,double* C,double* work1,double* work2,GridInfo3D* gi){
	dist_time = rd_time = bcast_time = cmp_time = 0;
	double w1 = MPI_Wtime();
	GridInfo gridX,gridY,gridZ;
	gridZ = gi->gz;
	int count = n*n;
...
//		}
	...
	if((gi->global).iam == 0){
		printf("whole = %f\n",w2-w1);
		printf("dist = %f, bcast = %f, rd = %f, cmp = %f\n",dist_time,bcast_time,rd_time,cmp_time);
	}
}
...
//	int err = MPI_Cart_create(oldComm,ndims,dims,period,reorder,&newComm);
	int err = MPIMY_Cart_create(oldComm,ndims,dims,period,&newComm);
	if(err != MPI_SUCCESS){
		fprintf(stderr,"cart create error\n");
		return -1;
	}
...
//	int err = MPI_Cart_create(oldComm,ndims,dims,period,reorder,&newComm);
	int err = MPIMY_Cart_create2(oldComm,ndims,dims,period,&newComm);
	if(err != MPI_SUCCESS){
		fprintf(stderr,"cart create error\n");
		return -1;
	}
...
//	int err = MPI_Cart_create(oldComm,ndims,dims,period,reorder,&newComm);
	int err = MPIMY_Cart_create_extra(oldComm,ndims,dims,period,&newComm);
	if(err != MPI_SUCCESS){
		fprintf(stderr,"cart create error\n");
		return -1;
	}
